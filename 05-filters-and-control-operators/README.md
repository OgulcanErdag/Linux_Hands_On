# Hands-on Linux-Plus-05 : Filters and Control Operators

​
Purpose of the this hands-on training is to teach the students how to use filters and control operators in Linux-Plus.
​
...
- Create a `text` file named `Linux-Plusway.txt`.
  ​

```bash
cat << EOF > Linux-Plusway.txt
Linux-Plusway:Road to reinvent yourself.
EOF
```

- Display the content of Linux-Plusway.txt.
  ​

```bash
cat Linux-Plusway.txt
```

- In the content of Linux-Plusway.txt, replace or translate aer letters with 'QAZ'.
  ​

```bash
cat Linux-Plusway.txt | tr aer QAZ
```
...
- Delete all the vowels in the content of Linux-Plusway.txt.
  ​

```bash
cat Linux-Plusway.txt | tr -d aeiou
```

​

- Write the whole content of Linux-Plusway.txt in capital letters.
  ​

```bash
cat Linux-Plusway.txt | tr [a-z] [A-Z]
```
...
- The command shell interprets the (||) as the logical `OR`. This is opposite of logical `AND`. Means second command will execute only when first command will be a failure.
  ​
- Display days.txt or write 'Linux-Plusway' on the screen, then write 'one'.
  ​

```bash
cat days.txt || echo Linux-Plusway ; echo one
```
...
- EXERCISE:
  ...
  1.a. Search for “Linux-Plusway.doc” in the current directory

      b. If it exists display its content

      c. If it does not exist print message “Too early!”

  2.Create a file named “Linux-Plusway.doc” that contains “Congratulations”
